    ...
    <xsl:template match="table_data[@name='opus']/row | table_data[@name='temp']/row">
        ...
        <xsl:element name="Opus_Document">
            ...
            <xsl:if test="string-length(field[@name='isbn']) > 0">
                <xsl:variable name="isbn_issn">
                    <xsl:value-of select="normalize-space(field[@name='isbn'])" />
                </xsl:variable>
                <xsl:choose>
                    <xsl:when test="string-length($isbn_issn) = 9 and string-length(substring-before($isbn_issn,'-')) = 4 and string-length(substring-after($isbn_issn,'-'))">
	   ...
	   <!-- TODO: Opus 4.1 Handle source as new Document -->
		...
		<xsl:choose>
		<xsl:when test="contains(field[@name='bem_extern'] , 'printed version not available')"/>
		<xsl:when test="contains(field[@name='bem_extern'] , 'no printed version')"/>
		<xsl:otherwise>
			<xsl:element name="Note">
			    <xsl:attribute name="Scope">
				<xsl:text>public</xsl:text>
