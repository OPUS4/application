    ...
    */
	...
	foreach ($doclist as $document)	{
            ...
            if ($document->getAttribute('name') === 'schriftenreihen') {
                $this->importSeries($document, $collRole);
            }
	}
   ...
   /**
    ...
    protected function importCollectionsDirectly($data, $collRole)  {
        ...
        try {
            ...
            foreach ($sorted_collections as $row) {
                ...
                // ZIB-Hack:
                if ($row['coll_name'] === 'ZIB') {
                    echo "Collection 'ZIB' skipped \n";
                    continue;
                }
    ...
    protected function importSeries($data, $role) {
        ...
        foreach ($classification as $class) {
            ...
            //echo ".";
            if ( ($class['name'] === 'ZIB-Report') || ($class['name'] === 'Jahresbericht')) {
                ...
                if ($class['name'] === 'ZIB-Report') { $zr_id  = $coll->getId();}
            }
            
            else {
                fputs($fp, $class['sr_id'] . ' ' . $zr_id . "\n");
                echo "Series " .  $class['name'] . " mapped to ZIB-Report.\n";
            }
