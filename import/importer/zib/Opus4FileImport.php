    ...
    */
    ...
    public function __construct($fulltextPath)  {
        ...
        try {
            ...
            if (!$this->_logfile) {
                throw new Exception("ERROR Opus4FileImport: Could not create '".$this->logfile."'\n");
            }
        } catch (Exception $e){
            echo $e->getMessage();
        }
	...
	$number = 0;
        ...
        //$opusId = $this->_tmpDoc->getIdentifierOpus3(0)->getValue();
    ...
    private function searchDir($from, $search) {
	    ...
	    */
            ...
            if ( is_dir($path) && $file === $search) {
                $this->_tmpPath = $path;
                $this->log("DEBUG Opus4FileImport: Directory for OpusId '".$search."' : ".$path."\n");
            }
    ...
    private function saveFiles()  {
        ...
        foreach ($this->_tmpFiles as $f) {
            ...
            if (basename($f) == 'index.html' || strpos(basename($f), '.') === 0) {
                $this->log("DEBUG Opus4FileImport: Skipped File '".basename($f)."'\n");
                continue;
            }

            // ERROR: File wirh same Basnemae already imported
            if (array_search(basename($f), $filesImported) !== false) {
                $this->log("ERROR Opus4FileImport: File '".basename($f)."' already imported.\n");
                continue;
            ...
            if (strrchr ($f, ".") === false) {
                $this->log("ERROR Opus4FileImport: File '".basename($f)."' has no extension and will be ignored.\n");
                continue;
            }
            /*
             * // TOOD in 4.1: Handle Label in index.xslt
            ...
            $this->log("DEBUG Opus4FileImport: File '".basename($f)."' imported.\n");

            $file = $this->_tmpDoc->addFile();
            $file->setPathName(basename($f));
            $file->setLabel(basename($f));
            $file->setTempFile($f);
            ...
            if (file_exists($comment_file)) {
                ...
                $this->log("DEBUG Opus4FileImport: Comment for File '".basename($f)."' imported.\n");
            }
