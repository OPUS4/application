...
class Opus3XMLImport {
    ...
    */
	...
	$this->validateEmails();
	    ...
	    // Test "nicht erschienen"
	    if ($doc->getTitleMain(0)->getValue() === '(nicht erschienen)') {
		throw new Exception("ERROR Opus3XMLImport: Title '".$doc->getTitleMain(0)->getValue()."' skipped.\n");
	    }
		...
		throw new Exception("ERROR Opus3XMLImport: Unpublished Document '".$doc->getTitleMain(0)->getValue()."' skipped.\n");
	    }

            // ThesisGrantor and ThesisPublisher only for Thesis-Documents
            if(in_array($doc->getType(), $this->thesistypes)) {
                //echo "ThesisType '".$doc->getType()."'\n";
            ...
            foreach ($this->collections as $c) {
                ...
                /* Examensarbeiten in der Collection "Dissertationen" wird der Dokumenttype 'doctoralthesis' zugeordnet */
                if (($coll->getName() === 'Dissertationen') && ($doc->getType() === 'masterthesis')) { $doc->setType('doctoralthesis'); }
            }
