 ...
 */
class DeliciousBookmarkForm extends Zend_Form
{
    /**
    ...
    public function init() {
		...
		$userTags->setLabel('delicious_usertags');

		$title = new Zend_Form_Element_Text('usertitle');
		$title->setRequired(true);
		$title->setValue($doc->getTitleMain(0)->getValue());
		$title->setLabel('delicious_usertitle');

		$description = new Zend_Form_Element_Textarea('userdescription');
		$description->setValue(substr($doc->getTitleAbstract(0)->getValue(), 0, 500));
		$description->setLabel('delicious_userdescription');

        $submit = new Zend_Form_Element_Submit('delicious_bm');
        $submit->setLabel('delicious_bookmark');
