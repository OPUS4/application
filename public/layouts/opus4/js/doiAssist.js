...
function finalize(field) {  // Grüne Farbe und Feldname wird in populatedFields geschrieben, um die Feldfarben nach einem Reload neu aufzubauen
    colorGreen(field);
...
function getLicence(json) {
    ...
    if (result != undefined) {finalize("Enrichmentlocal_crossrefLicence")}}
...
function getConferenceTitle(json) {
    ...
    if (result != undefined) {finalize("EnrichmentConferenceTitle")}}
    return result ? result : ''}
...
function getConferencePlace(json) {
    ...
    if (result != undefined) {finalize("EnrichmentConferencePlace")}}
    return result ? result : ''}
...
function getUrl(json) {
    ...
    if (result != undefined) {finalize("IdentifierUrl")}}
    return result ? result : ''}
...
function getIssn(json) {
    ...
    if (result != undefined) {finalize("IdentifierIssn")}}
    return result ? result : ''}
...
function getIsbn(json) {
    ...
    if (result != undefined) {finalize("IdentifierIsbn")}}
    return result ? result : ''}
...
function getVolume(json) {
    ...
    if (result != undefined) {finalize("Volume")}
    return result ? result : ''}
    ...
    if (_laenge > 0) {
        ...
        if (json.message.editor[_z].ORCID != null) {
            if (json.message.editor[_z].ORCID.includes("/")) {
                ...
                if (re != null){orcid = re[0];}
                else {orcid=''}
            }
            else {orcid = json.message.editor[_z].ORCID;}}
        else {orcid=''}
    ...
    if (_laenge > 0) {
        for (_z = 0; _z < _laenge; _z++) {
            ...
            if (json.message.author[_z].ORCID != null) {
                ...
                else {orcid = json.message.author[_z].ORCID;}}
            else {orcid=''}
            complete_name = nachname+','+vorname+','+orcid;    
...
function getTitleMain(json) {
    ...
    if (result != undefined) {finalize("TitleMain_1")}
    return result ? result : ''}
...
function getTitleSub(json) {
    ...
    if (result != undefined && result != '') {colorGreen("TitleSub_1")}
    return result ? result : ''}
...
function getLanguage(json) {
    ...
    if (result != undefined) {finalize("Language")} else {colorPink("Language")}
...
function getPublisherPlace(json) {
    ...
    if (result != undefined) {finalize("PublisherPlace")}
    return result ? result : '';
    }
