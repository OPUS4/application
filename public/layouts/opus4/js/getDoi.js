			...
			function startePruefung() {
				...
				if (doi.trim() == '') {alert("Bitte zuerst eine DOI eingeben...")} // Feld "IdentifierDoi" ist leer
				else if (doi.trim() != '' && document.getElementById("Enrichmentopus_doi_flag").value != "true") {leseDoi(doi)} // OK, starte Import
				...
				{ 
					//
					if (confirm("Achtung, alle Felder des Formulars werden gelöscht und ein neuer Import gestartet! Fortfahren?")) {aufraeumen(doi)}
					else {return}
				} 
			}
			...
			function leseDoi(doi) { // Diese Funktion wird beim Klick auf den Button "DOI-Daten übernehmen" aufgerufen und steuert alles Weitere									
			if (doi.trim() != '') {				
			...
			function parseJson(jsonraw) {
					...
					function expandCompletedDate(dates) { // Für CompletedYear und CompletedDate
		...
		function expandAuthor(author) {
			...
			else {colorPink("PersonAuthorLastName_1");}	
			}
		...
		async function getDoctypes(data) {
			...
			if (crossrefType == 'journal-article') {
				if (existingDoctypes.includes("article")) { document.getElementById("DocumentType").value = 'article'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			if (crossrefType == 'book') {
				if (existingDoctypes.includes("book")) { document.getElementById("DocumentType").value = 'book'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			...
			if (crossrefType == 'book-chapter') {
				if (existingDoctypes.includes("bookpart")) { document.getElementById("DocumentType").value = 'bookpart'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			...
			if (crossrefType == 'proceedings') {
				if (existingDoctypes.includes("conferenceobject")) { document.getElementById("DocumentType").value = 'conferenceobject'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			...
			if (crossrefType == 'journal') {
				if (existingDoctypes.includes("periodical")) { document.getElementById("DocumentType").value = 'periodical'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			if (crossrefType == 'journal-volume') {
				if (existingDoctypes.includes("periodicalpart")) { document.getElementById("DocumentType").value = 'periodicalpart'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			...
			if (crossrefType == 'posted-content/preprint') {
				if (existingDoctypes.includes("preprint")) { document.getElementById("DocumentType").value = 'preprint'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			if (crossrefType == 'report') {
				if (existingDoctypes.includes("report")) { document.getElementById("DocumentType").value = 'report'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			...
			if (crossrefType == 'peer-review') {
				if (existingDoctypes.includes("review")) { document.getElementById("DocumentType").value = 'review'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			if (crossrefType == 'book-track') {
				if (existingDoctypes.includes("sound")) { document.getElementById("DocumentType").value = 'sound'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			if (crossrefType == 'posted-content/working_paper') {
				if (existingDoctypes.includes("workingpaper")) { document.getElementById("DocumentType").value = 'workingpaper'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			//Wenn crossrefType nur "dissertation", ohne Slash mit Degree
			if (crossrefType == 'dissertation') {
				if (existingDoctypes.includes("doctoralthesis")) { document.getElementById("DocumentType").value = 'doctoralthesis'; }
				else { document.getElementById("DocumentType").value = 'other'; }
			}
			...
			if (crossrefType.includes("dissertation/")) { // Wenn crossrefType "dissertation" mit Slash: mit Degree
				...
				if (keys_master.some(el => degree.includes(el))) {
					if (existingDoctypes.includes("masterthesis")) { document.getElementById("DocumentType").value = 'masterthesis'; }
					else { document.getElementById("DocumentType").value = 'doctoralthesis'; }
				}
				else if (keys_bachelor.some(el_1 => degree.includes(el_1)) || degree === "ba") {
					if (existingDoctypes.includes("bachelorthesis")) { document.getElementById("DocumentType").value = 'bachelorthesis'; }
					else { document.getElementById("DocumentType").value = 'doctoralthesis'; }
				}
				else if (keys_habilitation.some(el_2 => degree.includes(el_2))) {
					if (existingDoctypes.includes("habilitation")) { document.getElementById("DocumentType").value = 'habilitation'; }
					else { document.getElementById("DocumentType").value = 'doctoralthesis'; }
				}
				else { document.getElementById("DocumentType").value = 'doctoralthesis'; }
			}
