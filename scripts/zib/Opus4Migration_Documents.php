...
require_once 'Opus4XMLImport.php';
require_once 'Opus4FileImport.php';

class Opus4Migration_Documents {
   ...
   // Import Documents
    public function load_documents() {
        $xmlImporter = new Opus4XMLImport($this->xslt, $this->stylesheet);
        ...
        foreach ($toImport as $document) {
            ...
            if ($result['result'] === 'success') {
                $xmlImporter->log(date('Y-m-d H:i:s') . " Successfully new ID " . $result['newid'] . " -- memory $mem_now (KB), peak memory $mem_peak (KB)\n");
                array_push($this->doclist, $result['newid']);
            } else if ($result['result'] === 'failure') {
                $xmlImporter->log(date('Y-m-d H:i:s') . " ERROR: " . $result['message'] . "\n" . $result['entry'] . "\n");
            }
        }
    ...
    public function load_fulltext() {
        ...
        $fileImporter = new Opus4FileImport($this->fulltextPath);
...
$migration = new Opus4Migration_Documents($options);
$status = $migration->run();

if ($status === Opus4Migration_Documents::_BREAK) { exit(1); }
