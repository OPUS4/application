...
index_runner='./solr_index_optimize.php'

stale_file='./index_optimize_cron.lock'
...
echo 'starting index optimize job @' `/bin/date '+%Y%m%d %H:%M:%S'` 2>&1 >> ${log_file}

${php_bin} ${index_runner} 2>&1 >> ${log_file}

echo 'index job optimize done @' `/bin/date '+%Y%m%d %H:%M:%S'` 2>&1 >> ${log_file}
