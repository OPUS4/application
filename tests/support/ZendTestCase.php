 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 *
 * Modified for testing OPUS 4 software with newer PHPUnit versions that use PHP namespaces.
 *
 * @phpcs:disable PSR2.Methods.MethodDeclaration
 */
abstract class ZendTestCase extends TestCase
{
    ...
    final public function bootstrap()
    {
        ...
        if (null !== $this->bootstrap) {
            if ($this->bootstrap instanceof Zend_Application) {
                ...
                $this->frontController = $this->bootstrap->getBootstrap()->getResource('frontcontroller');
    ...
    {
        if ($this->request instanceof Zend_Controller_Request_HttpTestCase) {
    ...
    public function assertXpath($path, $message = '')
    {
        ...
        require_once 'Zend/Test/PHPUnit/Constraint/DomQuery.php';
        $constraint = new Zend_Test_PHPUnit_Constraint_DomQuery($path);
        $constraint->registerXpathNamespaces($this->xpathNamespaces);
        $content = $this->response->outputBody();
        if (! $constraint->evaluate($content, __FUNCTION__)) {
            $constraint->fail($path, $message);
        }
