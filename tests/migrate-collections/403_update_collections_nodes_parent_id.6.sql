...
SELECT l.role_id AS role_id, l.id AS node_id, l.left_id, MAX(p.left_id) AS parent_left_id FROM collections_nodes AS l, collections_nodes AS p
 WHERE p.role_id = l.role_id AND l.role_id = 6
...
UPDATE collections_nodes, temp_2 SET collections_nodes.parent_id = temp_2.pid WHERE temp_2.nid = collections_nodes.id AND collections_nodes.role_id = 6;
